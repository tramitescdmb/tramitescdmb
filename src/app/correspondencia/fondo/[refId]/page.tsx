...
import { ArrowLeft, Archive, ExternalLink, FileWarning } from "lucide-react";
...
export default async function FichaFondoPage({
  params,
  searchParams,
}: {
  params: Promise<{ refId: string }>;
  searchParams: Promise<{ fondo?: string }>;
}) {
  ...
  const { fondo } = await searchParams;
  const FONDO: FondoId = fondo && esFondoValido(fondo) ? fondo : "psdocuments";
  ...
  const generales: [string, string | null][] = [
    ...(esPsdocuments ? ([["Serie", doc.serieNombre]] as [string, string | null][]) : []),
