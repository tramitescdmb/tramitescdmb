...
/** Descarga (URL firmada) de un documento de un expediente documental — con auditoría de LECTURA. */
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const doc = await db.documentoArchivo.findUnique({
    where: { id },
    select: { storagePath: true, nombre: true, expedienteDocumentalId: true },
  });
  ...
  await registrarAuditoriaDoc({
    ...
    ip,
    userAgent,
    detalle: `Descargó/abrió el documento "${doc.nombre}" del expediente ${doc.expedienteDocumentalId}`,
  });
