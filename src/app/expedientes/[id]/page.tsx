    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="mt-1 flex flex-wrap items-center gap-3">
          <h1 className="text-xl font-semibold text-gray-900">{expediente.numero}</h1>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          ...
          <section className="rounded-xl border border-gray-200 bg-white p-4">
            <h2 className="mb-2 text-sm font-semibold text-gray-900">Solicitante</h2>
            <dl className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm sm:grid-cols-4">
              <div>
                <dt className="text-xs text-gray-400">Nombre / razón social</dt>
                <dd className="text-gray-800">{expediente.solicitanteNombre}</dd>
              </div>
              <div>
                <dt className="text-xs text-gray-400">Identificación</dt>
                <dd className="text-gray-800">{expediente.solicitanteIdentificacion}</dd>
              </div>
              <div>
                <dt className="text-xs text-gray-400">Correo</dt>
                <dd className="text-gray-800">{expediente.solicitanteEmail ?? "—"}</dd>
              </div>
              <div>
          ...
          {pasoActual ? (
            <section className="rounded-xl border border-cdmb-300 bg-white p-5">
              <p className="mb-1 text-xs font-medium uppercase tracking-wide text-cdmb-600">
                Paso actual ({pasoActual.numero} de {pasos.length})
              </p>
              <h2 className="text-lg font-semibold text-gray-900">{pasoActual.titulo}</h2>
              <p className="mt-2 whitespace-pre-line text-sm text-gray-600">{pasoActual.descripcion}</p>
              ...
              <dl className="mt-3 grid grid-cols-1 gap-2 text-xs sm:grid-cols-3">
                {pasoActual.responsables.length > 0 && (
                  <div>
                    ...
                    <dd className="text-gray-700">{pasoActual.responsables.join(", ")}</dd>
                  </div>
                )}
                {pasoActual.documentos.length > 0 && (
                  <div>
                    <dt className="font-medium text-gray-500">📄 Documentos/registros de este paso</dt>
                    <dd className="text-gray-700">{pasoActual.documentos.join(", ")}</dd>
                  </div>
                )}
                {pasoActual.tiempo && (
                  <div>
                    <dt className="font-medium text-gray-500">🕒 Tiempo estimado</dt>
                    <dd className="text-gray-700">{pasoActual.tiempo}</dd>
                  </div>
                )}
              </dl>
              ...
              {pasos.map((p) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={estadoPaso === "completado" ? "line-through decoration-gray-300" : ""}>
                      {p.titulo}
                    </span>
                  </li>
                );
          ...
          <section className="rounded-xl border border-gray-200 bg-white p-4">
            <h2 className="text-sm font-semibold text-gray-900">Cambiar estado manualmente</h2>
            ...
            <form action={`/api/expedientes/${expediente.id}/estado`} method="post" className="flex flex-wrap items-end gap-3">
              <Field label="Nuevo estado" required help="">
                <select
                  ...
                  className="rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-cdmb-500 focus:outline-none focus:ring-1 focus:ring-cdmb-500"
                >
                  {ESTADOS.map((e) => (
              ...
              <Field label="Motivo (opcional)" help="Queda registrado en la bitácora del expediente.">
                <input
                  name="motivo"
                  className="w-64 rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-cdmb-500 focus:outline-none focus:ring-1 focus:ring-cdmb-500"
                />
              </Field>
              <button
                type="submit"
          ...
          <section>
            <h2 className="mb-2 text-sm font-semibold uppercase tracking-wide text-gray-500">
              Bitácora del expediente <span className="font-normal normal-case text-gray-400">— trazabilidad de todo lo que ha pasado</span>
            </h2>
            <form action={`/api/expedientes/${expediente.id}/comentario`} method="post" className="mb-3 flex gap-2">
              <input
                ...
                className="flex-1 rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-cdmb-500 focus:outline-none focus:ring-1 focus:ring-cdmb-500"
              />
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-gray-200 bg-white">
            <div className="border-b border-gray-100 px-4 py-3">
              ...
              <p className="text-xs text-gray-500">Todo lo adjuntado, de la radicación en adelante.</p>
            </div>
            {expediente.documentos.length === 0 ? (
              <p className="px-4 py-6 text-center text-sm text-gray-400">Sin documentos todavía.</p>
            ) : (
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-4 text-xs text-gray-500">
            ...
            <p>
              <span className="font-medium text-gray-700">Creado por:</span> {expediente.creadoPor.nombre}
            </p>
            {expediente.responsableActual && (
              <p>
                <span className="font-medium text-gray-700">A cargo de:</span> {expediente.responsableActual.nombre}
              </p>
            )}
          </div>
