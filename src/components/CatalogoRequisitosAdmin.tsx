...
/** Pantalla administrable del catálogo de requisitos documentales por etapa — antes solo se podía
    ...
    <div className="space-y-3">
      ...
      {ETAPAS_ORDEN.map((etapa) => {
        ...
        const grupos: { modalidad: ModalidadSeleccion | null; items: RequisitoCatalogo[] }[] = [
          ...
          ...ORDEN_MODALIDADES.map((m) => ({ modalidad: m, items: deEstaEtapa.filter((r) => r.modalidadSeleccion === m) })).filter((g) => g.items.length > 0),
        ];

        return (
          <details key={etapa} open className="rounded-xl border border-stone-200 bg-white p-4 shadow-sm">
              ...
              {grupos.map((g) => (
                <div key={g.modalidad ?? "comun"}>
                  <p className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-stone-400">
                    {g.modalidad ? ETIQUETA_MODALIDAD[g.modalidad] : "Común a todas las modalidades"}
                  </p>
                      ...
                      <li key={r.id} className={`flex flex-wrap items-center gap-2 p-2.5 text-sm ${r.activo ? "" : "opacity-50"}`}>
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="truncate text-xs text-stone-400">
                            ...
                            {r.fuente ?? "sin fuente registrada"}
                          </p>
                        </div>
                        <button
                          type="button"
      ...
      {agregarEn && <ModalAgregarRequisito etapa={agregarEn} onClose={() => setAgregarEn(null)} onSaved={() => { setAgregarEn(null); router.refresh(); }} />}
      {editando && <ModalEditarRequisito requisito={editando} onClose={() => setEditando(null)} onSaved={() => { setEditando(null); router.refresh(); }} />}
    </div>
  );
}

function ModalAgregarRequisito({ etapa, onClose, onSaved }: { etapa: EtapaContratacion; onClose: () => void; onSaved: () => void }) {
  const [nombre, setNombre] = useState("");
  const [codigoFormato, setCodigoFormato] = useState("");
  const [fuente, setFuente] = useState("");
  const [modalidadSeleccion, setModalidadSeleccion] = useState<string>("");
