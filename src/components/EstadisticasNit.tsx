      ...
      {error ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
            <Kpi icon={Building2} label="Total de terceros" value={num(datos.totalTerceros)} />
            <Kpi icon={CheckCircle2} label="Con al menos 1 vinculada" value={num(datos.conVinculacion)} />
            <Kpi icon={XCircle} label="Sin ninguna vinculada" value={num(datos.sinVinculacion)} />
            <Kpi
              icon={PieChart}
              label="% sin ninguna vinculada"
              value={`${(datos.porcentajeSinVinculacion * 100).toFixed(1)} %`}
